...
bool Assets::Apply() { return strategy_ ? strategy_->Apply(this) : false; }
...
bool Assets::Download(std::string url,
    ...
    while (true) {
        ...
        while (current_sector < needed_sectors) {
            ...
            ESP_LOGD(TAG, "Erasing sector %u (offset: %u, size: %u)", current_sector, sector_start,
                     SECTOR_SIZE);
        ...
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to write to assets partition at offset %u: %s", total_written,
                     esp_err_to_name(err));
            return false;
        }
        ...
        if (esp_timer_get_time() - last_calc_time >= 1000000 || total_written == content_length ||
            ...
            ESP_LOGI(TAG, "Progress: %u%% (%u/%u), Speed: %u B/s, Sectors erased: %u", progress,
                     total_written, content_length, speed, current_sector);
    ...
    if (total_written != content_length) {
        ESP_LOGE(TAG, "Downloaded size (%u) does not match expected size (%u)", total_written,
                 content_length);
        return false;
    }
